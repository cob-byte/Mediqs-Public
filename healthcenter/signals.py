...
def createPatient(sender, instance, created, **kwargs):
    if created:
        ...
        elif instance.is_healthcenter_admin:
            user = instance
            Admin_Information.objects.create(
                user=user, username=user.username, email=user.email)
        elif instance.is_inventorymanager:
            user = instance
            InventoryManager.objects.create(user=user, username=user.username, email=user.email)
...
def updateUser(sender, instance, created, **kwargs):
    ...
    if created == False:
        user.first_name = patient.username
